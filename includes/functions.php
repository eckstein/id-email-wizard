...
// Folder breadcrumb and folder <select> generator are now in
// includes/folders/helpers.php.
 ...
 */
function get_idwiz_metric_rates($campaignIds = [], $startDate = null, $endDate = null, $campaignTypes = ['Blast', 'Triggered', 'FromWorkflow'], $purchaseMode = 'campaignsInDate')
{
	...
	$totalRevenue = $totalPurchases > 0 ? array_sum(array_column($purchases, 'total')) : 0;
	...
	return [
		...
		'wizCtr' => ($totalClicks > 0 && $totalSends > 0) ? ($totalClicks / $totalSends) * 100 : 0,
		'wizCto' => ($totalOpens > 0 && $totalClicks > 0) ? ($totalClicks / $totalOpens) * 100 : 0,
		...
		'wizUnsubRate' => ($totalUnsubscribes > 0 && $totalSends > 0) ? ($totalUnsubscribes / $totalSends) * 100 : 0,
		'wizCompRate' => ($totalComplaints > 0 && $totalSends > 0) ? ($totalComplaints / $totalSends) * 100 : 0,
...
function get_triggered_campaign_metrics($campaignIds = [], $startDate = null, $endDate = null)
{
	...
	// Convert single campaign ID to array for consistency
...
function get_triggered_campaign_metrics_chunk($campaignIds, $startDate, $endDate)
{
	...
	// Build comprehensive list of all campaign IDs including connected ones
	$allCampaignIds = [];
	foreach ($allCampaigns as $campaign) {
		$allCampaignIds[] = $campaign['id'];
		$connectedCampaigns = unserialize($campaign['connectedCampaigns']);
		if ($connectedCampaigns && is_array($connectedCampaigns)) {
			$allCampaignIds = array_merge($allCampaignIds, $connectedCampaigns);
		}
	}
	$allCampaignIds = array_unique($allCampaignIds);

	// Batch fetch all purchases for all campaigns at once
	$purchasesOptions = [
		'startAt_start' => $startDate,
		'startAt_end' => $endDate,
		'campaignIds' => $allCampaignIds,
	...
	// Batch fetch all GA data for all campaigns at once
	$allGaEmailPurchases = get_idwiz_ga_data(['startDate' => $startDate, 'endDate' => $endDate, 'campaignIds' => $allCampaignIds]);

	// Batch query arguments for all triggered data
	$campaignDataArgs = [
		'campaignIds' => $allCampaignIds,
		'startAt_start' => $startDate,
		'startAt_end' => $endDate,
