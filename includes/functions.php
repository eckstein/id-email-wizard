...
function idemailwiz_generate_chunk_html() {
	...
	if ( $builderVersion == 'v2' ) {
		$chunkFileBase = '/builder-v2';
	} else {
		$chunkFileBase = '/templates';
	}
	...
	if ( isset( $chunks[ $row_index ] ) ) {
		...
		$file = dirname( plugin_dir_path( __FILE__ ) ) . $chunkFileBase . '/chunks/' . $chunkFileName . '.php';
...
// Generate a chunk's HTML
function idwiz_get_chunk_template( $chunk, $variant = null, $parentFieldId = null, $rowId = null, $forPreview = false ) {
	$chunkType = $chunk['acf_fc_layout'];

	$return = '';
	if ( $variant && $forPreview) {
		$return .= '<div class="child-chunkWrap" data-parent-field-id="'.$parentFieldId.'" data-content-index="'.trim($rowId).'">';
	}
	if ($chunkType == 'plain_text') {
		$return .= idwiz_get_plain_text_chunk( $chunk );
	} else if ($chunkType == 'image') {
		$return .= idwiz_get_image_chunk( $chunk, $variant );
	} else if ($chunkType == 'button') {
		$return .= idwiz_get_button_chunk( $chunk );
	} else if ($chunkType == 'two_column') {
		$return .= idwiz_get_two_column_chunk( $chunk, $forPreview );
	} else if ($chunkType == 'three_column') {
		$return .= idwiz_get_three_column_chunk( $chunk, $forPreview );
	} else if ($chunkType =='spacer') {
		$return .= idwiz_get_spacer_chunk( $chunk );
	} else if ($chunkType == 'html') {
		$return .= idwiz_get_html_chunk( $chunk );
	}
...
function idwiz_assemble_template_html($chunks, $templateSettings, $templateStyles, $emailSettings) {

  $externalUTMs = $emailSettings['external_utms'];
  $extUTMstring = $emailSettings['external_utm_string'];

  $htmlOutput = ''; // Initialize an empty string to accumulate HTML content

  // Email top
  $htmlOutput .= idwiz_get_email_top($chunks, $templateSettings, $templateStyles, $emailSettings);

  // iD Logo Header
  if ($templateSettings['id_tech_header'] == true) {
      $htmlOutput .= idwiz_get_standard_header();
  }

  $chunksHtml = '';
  if (!empty($chunks)) {
    foreach ($chunks as $chunk) {
      $chunkHtml = idwiz_get_chunk_template($chunk);

      if ($externalUTMs) {
        $chunkHtml = idwiz_add_utms($chunkHtml, $extUTMstring);
      }
  ...
  $htmlOutput .= $chunksHtml;

  // Email Footer
  if ($templateSettings['id_tech_footer'] == true) {
    $showUnsub = $templateSettings['show_unsubscribe_link'] != true ? false : true;
    $htmlOutput .= idwiz_get_standard_footer($showUnsub);
  }

  // Fine print/disclaimer
  if (!empty($templateSettings['fine_print_disclaimer'])) {
    $htmlOutput .= idwiz_get_fine_print_disclaimer($templateSettings['fine_print_disclaimer']);
  }

  // Email Bottom
  $htmlOutput .= idwiz_get_email_bottom();

  return $htmlOutput; // Return the accumulated HTML
}
...
function idemailwiz_generate_template_html() {
  ...
  if ( $builderVersion == 'v2' ) {
    $chunkFileBase = '/builder-v2';
    $html = idwiz_assemble_template_html( $chunks, $templateSettings, $templateStyles, $emailSettings );
	if ($mode == 'code') {
		echo htmlspecialchars(html_entity_decode($html));
	} else {
		echo '<div class="fullModeViewWrapper">';
		echo $html; // Send the rendered HTML
		echo '</div>';
	}
    wp_die();
    return;
  ...
  $extUTMstring = $emailSettings['external_utm_string'];
	...
	include( dirname( plugin_dir_path( __FILE__ ) ) . $chunkFileBase . '/chunks/email-top.php' );
	include( dirname( plugin_dir_path( __FILE__ ) ) . $chunkFileBase . '/chunks/css.php' );
	include( dirname( plugin_dir_path( __FILE__ ) ) . $chunkFileBase . '/chunks/end-email-top.php' );


	//Standard email header snippet, if active
	if ( $templateSettings['id_tech_header'] == true ) {
		include( dirname( plugin_dir_path( __FILE__ ) ) . $chunkFileBase . '/chunks/standard-email-header.php' );
	}
	...
	if ( $templateSettings['id_tech_footer'] == true ) {
		...
		include( dirname( plugin_dir_path( __FILE__ ) ) . $chunkFileBase . '/chunks/standard-email-footer.php' );
	}

	//Fine print/disclaimer text
	if ( ! empty( $templateSettings['fine_print_disclaimer'] ) ) {
		include( dirname( plugin_dir_path( __FILE__ ) ) . $chunkFileBase . '/chunks/email-before-disclaimer.php' );
		include( dirname( plugin_dir_path( __FILE__ ) ) . $chunkFileBase . '/chunks/fine-print-disclaimer.php' );
		include( dirname( plugin_dir_path( __FILE__ ) ) . $chunkFileBase . '/chunks/email-after-disclaimer.php' );
	}
