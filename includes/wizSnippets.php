...
function get_snippet_css($chunks) {
    $css = '';

    foreach ($chunks as $chunk) {
        if (isset($chunk['acf_fc_layout']) && $chunk['acf_fc_layout'] == 'html') {
            // Assuming 'snippet' is the sub-field with the post ID
            $snippet_id = $chunk['snippet'] ?? null;
            if ($snippet_id) {
